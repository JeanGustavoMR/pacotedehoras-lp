  ...
  return (
    <div className="relative" ref={dropdownRef}>
      <button
        ...
        className="flex items-center gap-2 px-4 py-2.5 rounded-xl bg-white/10 backdrop-blur-sm hover:bg-white/20 transition-all duration-300 text-sm font-medium border border-white/20 hover:border-white/30 shadow-sm hover:shadow-md"
      >
      ...
      {isOpen && (
        <div className="absolute right-0 top-full mt-3 w-44 sm:w-52 bg-white/95 backdrop-blur-md border border-white/30 rounded-2xl shadow-xl overflow-hidden z-50 animate-in slide-in-from-top-2 duration-200">
          {languages.map((language) => (
            <button
              key={language.code}
              onClick={() => handleLanguageChange(language.code)}
              className={`w-full flex items-center gap-3 px-4 py-3.5 text-left hover:bg-primary/10 transition-all duration-200 ${
